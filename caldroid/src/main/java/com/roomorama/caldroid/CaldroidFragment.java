...
public class CaldroidFragment extends DialogFragment {
	 ...
	 * @throws ParseException
	 */
	public void setSelectedDateStrings(List<String> dateList,String dateFormat) throws ParseException {
		...
		for(String strDate : dateList) {
			Date date = CalendarHelper.getDateFromString(strDate, dateFormat);
			DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		customTitleBar = (RelativeLayout)view.findViewById(R.id.calendar_title_layout);
		customTitle = (TextView)view.findViewById(R.id.calendar_dialog_title);
	...
	public class DatePageChangeListener implements OnPageChangeListener {
		...
		public void refreshAdapters(int position) {
			...
			int offset = Math.abs(currentPage - position);
			...
			// Swipe right
				...
				currentDateTime = currentDateTime.plus(0, offset, 0, 0, 0, 0, 0,
						DateTime.DayOverflow.LastDay);

				// Refresh the adapter of next gridview
				nextAdapter.setAdapterDateTime(currentDateTime.plus(0, offset, 0, 0,
						0, 0, 0, DateTime.DayOverflow.LastDay));
				nextAdapter.notifyDataSetChanged();
			...
			// Swipe left
				...
				currentDateTime = currentDateTime.minus(0, offset, 0, 0, 0, 0, 0,
						DateTime.DayOverflow.LastDay);

				// Refresh the adapter of previous gridview
				prevAdapter.setAdapterDateTime(currentDateTime.minus(0, offset, 0,
						0, 0, 0, 0, DateTime.DayOverflow.LastDay));
				prevAdapter.notifyDataSetChanged();
			}
