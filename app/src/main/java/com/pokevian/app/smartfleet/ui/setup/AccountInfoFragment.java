...
public class AccountInfoFragment extends Fragment implements OnClickListener, OnItemClickListener {
    ...
    private UpdateAccountCallback mCallbacks;
    ...
    public void onAttach(Activity activity) {
        ...
        try {
            mCallbacks = (UpdateAccountCallback) getParentFragment();
        } catch (ClassCastException e) {
            throw new ClassCastException(activity + " must implement RegisterAccountCallbacks");
        }
        if (mCallbacks == null) {
            try {
                mCallbacks = (UpdateAccountCallback) activity;
            } catch (ClassCastException e) {
                throw new ClassCastException(activity + " must implement RegisterAccountCallbacks");
            }
        }
...
//    }
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        mNextBtn.setText(R.string.btn_modify);
        mNextBtn.setOnClickListener(this);
        mNextBtn.setEnabled(false);

        if (getArguments() == null) {
            AccountItem item = mAdapter.getItem(ITEM_NICK_NAME);
            item.value = SettingsStore.getInstance().getAccountName();

            item = mAdapter.getItem(ITEM_EMAIL);
            item.value = SettingsStore.getInstance().getLoginId();

            Account account = SettingsStore.getInstance().getAccount();
            item = mAdapter.getItem(ITEM_BIRTHDAY);
            item.date = account.getBirthday();
            item.value = DateFormat.getDateFormat(getActivity()).format(item.date);

            item = mAdapter.getItem(ITEM_REGION);
            item.value = getRegionName(account.getRegionCode());

            item = mAdapter.getItem(ITEM_SEX);
            item.value = account.getSexCode().equals(SexCd.male.name()) ? getString(R.string.sign_up_male): getString(R.string.sign_up_female);
        }
    }
...
//        for (int i = ITEM_NICK_NAME; i < ITEM_COUNT; i++) {
//            AccountItem item = mAdapter.getItem(i);
...
//        }
    ...
    public void onNickNameInput(DialogFragment fragment, String nickName) {
        ...
        mNextBtn.setEnabled(!TextUtils.isEmpty(nickName));
...
//        item.value = null;
    ...
    public void onSignUpSuccess(DialogFragment fragment, Account account) {
        logger.trace("onSignUpSuccess(): account=" + account);

        mCallbacks.onUpdate(account);
    }
...
//                    return R.string.sign_up_country;
    ...
    public interface UpdateAccountCallback {
        void onUpdate(Account newAccount);
    }
            ...
            return new AlertDialog.Builder(getActivity())
                    ...
                    .setPositiveButton(R.string.btn_ok, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            ((AccountInfoFragment) getParentFragment())
    ...
    public static class PickBirthdayDialogFragment extends DialogFragment implements OnDateSetListener {
        ...
        public void onDateSet(DatePicker view, int year, int monthOfYear,
            ...
            ((AccountInfoFragment) getParentFragment()).onBirthdaySelect(this, birthday.getTime());
        }
